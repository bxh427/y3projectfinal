...
This code produces Poincare data for single alpha for a billiard on a rectangular table of user-defined size.
...
int main()
{
	...
	ofstream gnu;
	ofstream outs;
	outs.open("poincareRectangle.txt");
	gnu.open("gnupoincareRectangle.txt");
	...
	for(int n=1;n<=bounces;n++)
	{
		...
		// Print results
		outs << setw(10) << n
			 << setw(25) << setprecision(12) << getTheta(InitPos.ycomp,InitPos.xcomp,a,b)
			 << setw(25) << getAlpha(InitPos, Norm, V_in, a, b) << endl;
		gnu << setw(25) << setprecision(12) << getTheta(InitPos.ycomp,InitPos.xcomp,a,b)
			<< setw(25) << getAlpha(InitPos, Norm, V_in, a, b) << endl;
	...
	cout << "\nResults complete.\n"
		 << "To view this data, open 'poincareRectangle.txt'.\n"
		 << "To plot this data, open 'rectpoinplot.gnu' and set a and b equal to the same height and width respectively."
...
double getAlpha(Vector pos, Vector NORM, Vector vel, double A, double B)
{
	...
	else if(pos.xcomp == B)
	{
		...
		{
			ALPHA = (asin(-1*DotProduct(vel,NORM)/sqrt(DotProduct(vel,vel))));
		}
	}
