...
Bunimovich Stadium
This code produces Poincare data for a billiard on a stadium shaped table of user-defined size.
...
double getAlpha(Vector pos, Vector NORM, Vector vel, double A, double B);
...
int main()
{
	...
	gnu.open("gnupoincareStadium.txt");
	...
	outs << "\n" << "For a billiard ball starting at position ("
		 ...
		 << V_in.xcomp << "," << V_in.ycomp << ") the following data is produced: " << endl
		 << setw(25) << "Bounce"
		 << setw(25) << "theta"
		 << setw(25) << "alpha" << endl << setprecision(15)
		 << setw(25) << "0"
		 << setw(25) << getTheta(InitPos.ycomp,InitPos.xcomp,a,b)
		 << setw(25) << getAlpha(InitPos, Norm, V_in, a, b) << endl;
	gnu	<< setw(25) << "#theta"
		<< setw(25) << "alpha" << endl
		<< setw(25) << getTheta(InitPos.ycomp,InitPos.xcomp,a,b)
		<< setw(25) << getAlpha(InitPos, Norm, V_in, a, b) << endl;
	...
	for(int n=1;n<=bounces;n++)
	{
		...
		outs << setw(25) << n
			 << setw(25) << getTheta(InitPos.ycomp,InitPos.xcomp,a,b)
			 << setw(25) << getAlpha(InitPos, Norm, V_in, a, b)<< endl;
		gnu << setw(25) << setprecision(15) << getTheta(InitPos.ycomp,InitPos.xcomp,a,b)
			<< setw(25) << getAlpha(InitPos, Norm, V_in, a, b) << endl;
	...
	cout << "\nResults complete.\n"
		 << "To view this data, open 'poincareStadium.txt'. \n"
		 << "To plot this data, open 'stadpoinplot.gnu' and set a and b equal to the same height and width respectively."
...
{
	double ALPHA;
	double rawgrad = vel.ycomp/vel.xcomp;
	double grad = round(rawgrad*10000)/10000;
	double rawgradN = NORM.ycomp/NORM.xcomp;
	double gradN = round(rawgradN*10000)/10000;
	double gradT = -1/gradN;
	double pi = acos(-1);
	...
	if(pos.ycomp == A)
	{
		...
		{
			ALPHA = (acos(-1*DotProduct(vel,NORM)/sqrt(DotProduct(vel,vel))));
		}
		
		else if(vel.xcomp < 0)
		{
			ALPHA = -1*(acos(-1*DotProduct(vel,NORM)/sqrt(DotProduct(vel,vel))));
		}
	}
	...
	// D
	else if(pos.xcomp >0 && pos.ycomp > 0.5*A)
	{
		if(grad == gradN)
		{
			ALPHA = pi/2;
		...
		else if((grad < gradN && grad >0) || (grad > gradT && grad < 0 && grad != gradN))
		{
			ALPHA = (acos(-1*DotProduct(vel,NORM)/sqrt(DotProduct(vel,vel))));
		}

		else if(grad > gradN || (grad < gradT && grad != gradN))
		{
			ALPHA = -1*(acos(-1*DotProduct(vel,NORM)/sqrt(DotProduct(vel,vel))));
		}
	}
